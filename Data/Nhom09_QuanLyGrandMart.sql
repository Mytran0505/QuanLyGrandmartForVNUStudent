...
--TRIGGER
CREATE OR REPLACE TRIGGER TRIGGER_POINT_INS
...
END;
/
CREATE OR REPLACE TRIGGER TRIGGER_POINT_UPD 
AFTER UPDATE OF TOTAL_MONEY ON BILL
FOR EACH ROW
BEGIN
    IF (:NEW.TOTAL_MONEY < :OLD.TOTAL_MONEY) THEN
        UPDATE STUDENT
        SET POINT = POINT - (TRUNC(:OLD.TOTAL_MONEY/100000)-TRUNC(:NEW.TOTAL_MONEY/100000))
        WHERE STUDENT_ID=:NEW.STUDENT_ID;
        
        UPDATE STUDENT
        SET SPENT_MONEY = SPENT_MONEY - (:OLD.TOTAL_MONEY - :NEW.TOTAL_MONEY)
        WHERE STUDENT_ID=:NEW.STUDENT_ID;
    ELSIF (:NEW.TOTAL_MONEY > :OLD.TOTAL_MONEY) THEN
        UPDATE STUDENT
        SET POINT = POINT + (TRUNC(:NEW.TOTAL_MONEY/100000)-TRUNC(:OLD.TOTAL_MONEY/100000))
        WHERE STUDENT_ID=:NEW.STUDENT_ID;
        
        UPDATE STUDENT
        SET SPENT_MONEY = SPENT_MONEY + (:NEW.TOTAL_MONEY - :OLD.TOTAL_MONEY)
        WHERE STUDENT_ID=:NEW.STUDENT_ID;
    END IF;
END;
/
...
END;
/
CREATE OR REPLACE TRIGGER TRIGGER_REMAINING_QUANTITY_INS
...
END;
/
CREATE OR REPLACE TRIGGER TRIGGER_REMAINING_QUANTITY_UPD
AFTER UPDATE OF AMOUNT ON BILL_DETAILS
FOR EACH ROW
BEGIN
    IF(:NEW.AMOUNT > :OLD.AMOUNT)THEN
        UPDATE PRODUCT
        SET REMAINING_QUANTITY = REMAINING_QUANTITY - (:NEW.AMOUNT-:OLD.AMOUNT)
        WHERE PRODUCT_ID = :NEW.PRODUCT_ID;
    ELSIF (:NEW.AMOUNT < :OLD.AMOUNT) THEN
        UPDATE PRODUCT
        SET REMAINING_QUANTITY = REMAINING_QUANTITY + (:OLD.AMOUNT-:NEW.AMOUNT)
        WHERE PRODUCT_ID = :NEW.PRODUCT_ID;
    END IF;
END;
